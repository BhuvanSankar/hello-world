...
import server.PairDist.EditDistanceResult;
import server.GettingTreadsDescription;
...
public class AllPairWise {
	...
	public static void Clustering() throws Exception{
		GettingTreadsDescription.DestReads();
		descript = GettingTreadsDescription.GetRname();
		reads = GettingTreadsDescription.GetTReads();
		Nreads = GettingTreadsDescription.NumberReads();
		FileName = GettingTreadsDescription.GetFileName();
		NumberElements = GettingTreadsDescription.SeqLength();
		ClustElements = GettingTreadsDescription.NelementsClustering();			
		...
		for (int i = 0; i < reads.size(); i++) {
			for (int j = i + 1; j < reads.size(); j++) {
				...
				EditDistanceResult result = PairDist.compute(x, y);
